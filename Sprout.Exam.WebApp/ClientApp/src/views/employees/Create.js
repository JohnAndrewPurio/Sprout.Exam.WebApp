...
export class EmployeeCreate extends Component {
                ...
                <form>
                    <div className='form-row'>
                        ...
                        <div className='form-group col-md-6'>
                            ...
                            <input type='date' className='form-control' id='inputBirthdate4' onChange={this.handleChange.bind(this)} name="birthdate" value={this.state.birthdate} placeholder='Birthdate' max={new Date().toLocaleDateString("en-ca")} />
                        </div>
                    </div>
                    <div className="form-row">
                        <div className='form-group col-md-6'>
    ...
    async saveEmployee() {
        ...
        const { fullName, birthdate, tin, typeId } = this.state;
        const token = await authService.getAccessToken();
        const requestOptions = {
            method: 'POST',
            headers: !token ? {} : { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({
                fullName, birthdate, tin, typeId
            })
        };
        ...
        if (response.status === 201) {
            ...
            alert(await response.text() || "An error occurred");
        }
    }
